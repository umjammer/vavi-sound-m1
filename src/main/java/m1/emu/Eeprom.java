...
public class Eeprom {
    ...
    private static boolean eeprom_command_match(byte[] buf, byte[] cmd, int len) {
        int ibuf = 0, idx = 0;
        if (cmd.length == 0) {
            return false;
        }
    ...
    public static void SaveStateBinary(BinaryWriter writer) {
        ...
        writer.write(clock_line.v);
        writer.write(reset_line.v);
        writer.write(locked);
    ...
    public static void LoadStateBinary(BinaryReader reader) throws IOException {
        ...
        clock_line = LineState.values()[reader.readInt32()];
        reset_line = LineState.values()[reader.readInt32()];
