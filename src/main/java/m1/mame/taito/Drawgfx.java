...
public class Drawgfx {
    ...
    public static void common_drawgfx_opwolf(byte[] bb1, int code, int color, int flipx, int flipy, int sx, int sy, RECT clip, int pri_mask) {
        int ox;
        int oy;
        int ex;
        int ey;
        ...
        for (i = 0; i < dstheight; i++) {
            for (j = 0; j < dstwidth; j++) {
                ...
                if (col != 0) {
                    ...
                    {
                        Video.bitmapbase[Video.curbitmap][(offsety + ydir * i) * 0x140 + offsetx + xdir * j] = (short) (colorbase + col);
                    }
