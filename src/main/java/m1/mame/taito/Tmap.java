                ...
                if (prev_trans != trans_t.WHOLLY_TRANSPARENT) {
                    ...
                    if (prev_trans == trans_t.WHOLLY_OPAQUE) {
                        for (cury = y; cury < nexty; cury++) {
                            System.arraycopy(pixmap, offsety2 * width + x_start, Video.bitmapbase[Video.curbitmap], (offsety2 + ypos) * 0x140 + xpos + x_start, x_end - x_start);
                            if (priority != 0) {
                                for (i = xpos + x_start; i < xpos + x_end; i++) {
                                    Tilemap.priority_bitmap[offsety2 + ypos][i] = (byte) (Tilemap.priority_bitmap[offsety2 + ypos][i] |
                                            priority);
                                }
                            }
                            offsety2++;
