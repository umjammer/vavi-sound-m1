 ...
 *  logger.log(Level.TRACE, str);
 * </pre>
 * please note that however much your tempted to,
 * timings can't be put in a table here because they depend on how the instruction executes at runtime
 */
public class Disassembler {

    public static final byte[][] opcodeSizes = new byte[9][256];
    ...
    public static void GenerateOpcodeSizes() {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[0][i] = (byte) pc.get();
        }
        ...
        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xCB, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[1][i] = (byte) pc.get();
        }

        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xED, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[2][i] = (byte) pc.get();
        }

        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xDD, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[3][i] = (byte) pc.get();
        }
        ...
        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xFD, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[4][i] = (byte) pc.get();
        }
        ...
        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xDD, (byte) 0xCB, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[5][i] = (byte) pc.get();
        }

        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xFD, (byte) 0xCB, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[6][i] = (byte) pc.get();
        }

        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xDD, (byte) 0xED, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[7][i] = (byte) pc.get();
        }

        for (int i = 0; i < 256; i++) {
            AtomicInteger pc = new AtomicInteger(0);
            byte[] opcode = {(byte) 0xFD, (byte) 0xED, (byte) i, 0, 0, 0};
            disasm.Disassemble(() -> opcode[pc.getAndIncrement()]);
            opcodeSizes[8][i] = (byte) pc.get();
        }
    }

    static String Result(String format, Supplier<Byte> read) {
        ...
        if (format.contains("nn")) {
            byte B = read.get();
            byte C = read.get();
            format = format.replace("nn", "%4Xh".formatted(B + C * 256));
        }

        if (format.contains("n")) {
            byte B = read.get();
            format = format.replace("n", "%2Xh".formatted(B));
        }

        if (format.contains("+d")) format = format.replace("+d", "d");

        if (format.contains("d")) {
            byte B = read.get();
            boolean neg = ((B & 0x80) != 0);
            char sign = neg ? '-' : '+';
            int val = neg ? 256 - B : B;
            format = format.replace("d", "%s%2Xh".formatted(sign, val));
        }
    ...
    final static String[] mnemonicsDD = {
            "NOP", "LD BC, nn", "LD (BC), A", "INC BC", //0x04
            "INC B", "DEC B", "LD B, n", "RLCA", //0x08
    ...
    final static String[] mnemonicsFD = {
            "NOP", "LD BC, nn", "LD (BC), A", "INC BC", //0x04
            "INC B", "DEC B", "LD B, n", "RLCA", //0x08
    ...
    final static String[] mnemonicsDDCB = {
    ...
    final static String[] mnemonicsFDCB = {
    ...
    final static String[] mnemonicsCB = {
    ...
    final static String[] mnemonicsED = {
    ...
    String DisassembleInternal(Supplier<Byte> read) {
        byte A = read.get();
        String format;
        switch (A & 0xff) {
            case 0xCB:
    ...
    public String Disassemble(Supplier<Byte> read) {
    ...
    static String Result2(String format, byte[] code, int offset) {
        ...
        if (format.contains("d")) {
            ...
            char sign = neg ? '-' : '+';
            int val = neg ? 256 - B : B;
            format = format.replace("d", "%c%2Xh".formatted(sign, val));
        }
    ...
    public String GetDisassembleInfo(short short1) {
        ...
        String sDisassemble;
        byte1 = Z80A.zz1[0].ReadOp.apply(short1);
        if (byte1 != (byte) 0xCB && byte1 != (byte) 0xED && byte1 != (byte) 0xDD && byte1 != (byte) 0xFD) {
            opcode2 = new byte[] {byte1, Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 1)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3))};
            length = Disassembler.opcodeSizes[0][byte1];
            offset = 1;
        } else if (byte1 == (byte) 0xCB) {
            opcode2 = new byte[] {byte1, Z80A.zz1[0].ReadOp.apply((short) (short1 + 1)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4))};
            length = Disassembler.opcodeSizes[1][byte1 & 0xff];
            offset = 2;
        } else if (byte1 == (byte) 0xED) {
            opcode2 = new byte[] {byte1, Z80A.zz1[0].ReadOp.apply((short) (short1 + 1)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4))};
            length = Disassembler.opcodeSizes[2][byte1 & 0xff];
            offset = 2;
        } else if (byte1 == (byte) 0xDD) {
            byte2 = Z80A.zz1[0].ReadOp.apply((short) (short1 + 1));
            if (byte2 != (byte) 0xCB && byte2 != (byte) 0xED) {
                opcode2 = new byte[] {byte1, byte2, Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4))};
                length = Disassembler.opcodeSizes[3][byte2];
                offset = 2;
            } else if (byte2 == (byte) 0xCB) {
                byte3 = Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2));
                opcode2 = new byte[] {byte1, byte2, byte3, Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 5))};
                length = Disassembler.opcodeSizes[5][byte3];
                offset = 2;
            } else if (byte2 == (byte) 0xED) {
                byte3 = Z80A.zz1[0].ReadOp.apply((short) (short1 + 2));
                opcode2 = new byte[] {byte1, byte2, byte3, Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 5))};
                length = Disassembler.opcodeSizes[7][byte3];
                offset = 2;
            }
        } else if (byte1 == (byte) 0xFD) {
            byte2 = Z80A.zz1[0].ReadOp.apply((short) (short1 + 1));
            if (byte2 != (byte) 0xCB && byte2 != (byte) 0xED) {
                opcode2 = new byte[] {byte1, byte2, Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 2)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 3)), Z80A.zz1[0].ReadOpArg.apply((short) (short1 + 4))};
        ...
        System.arraycopy(opcode2, 0, opcode, 0, length);
        s2 = Disassemble2(opcode, offset);
        for (i = 0; i < length; i++) {
            s1.append("%2X".formatted(opcode[i])).append(" ");
        }
        sDisassemble = "%-20s".formatted(s1.toString()) + s2;
        return sDisassemble;
    }
}
