...
public class YMDeltat {
    ...
    public static class YM_DELTAT {
        ...
        public byte memread;
        // status_callback
        public Consumer<Byte> status_set_handler;
        // status_callback
        public Consumer<Byte> status_reset_handler;
    ...
    private static final byte[] dram_rightshift = {3, 0, 0, 0};
                ...
                if ((DELTAT.portstate & 0x01) != 0) {
                    ...
                    if (DELTAT.status_set_handler != null) {
                        if (DELTAT.status_change_BRDY_bit != 0) {
                            DELTAT.status_set_handler.accept(DELTAT.status_change_BRDY_bit);
                        }
                    }
                }
                break;
                ...
                if ((DELTAT.portstate & 0xe0) == 0x60) {
                    ...
                    if (DELTAT.now_addr != (DELTAT.end << 1)) {
                        ...
                        if (DELTAT.status_reset_handler != null) {
                            if (DELTAT.status_change_BRDY_bit != 0) {
                                DELTAT.status_reset_handler.accept(DELTAT.status_change_BRDY_bit);
                            }
                        }
                        if (DELTAT.status_set_handler != null) {
                            if (DELTAT.status_change_BRDY_bit != 0) {
                        ...
                        if (DELTAT.status_set_handler != null) {
                            if (DELTAT.status_change_EOS_bit != 0) {
                                DELTAT.status_set_handler.accept(DELTAT.status_change_EOS_bit);
                            }
                        }
                    }
                    return;
    ...
    private static void YM_DELTAT_synthesis_from_external_memory() {
        ...
        DELTAT.adpcml = DELTAT.prev_acc * ((1 << 16) - DELTAT.now_step);
        DELTAT.adpcml += (DELTAT.acc * DELTAT.now_step);
        DELTAT.adpcml = (DELTAT.adpcml >> 16) * DELTAT.volume;
