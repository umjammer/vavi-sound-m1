...
public class Iremga20 {
    ...
    public static class IremGA20_channel_def {
    ...
    public static class IremGA20_chip_def {
    ...
    public static void irem_ga20_w(int offset, short data) {
        int channel;
        ...
        switch (offset & 0x7) {
            case 0:
                chip.channel[channel].start = ((chip.channel[channel].start) & 0xf_f000) | (data << 4);
                break;
            case 1:
                chip.channel[channel].start = ((chip.channel[channel].start) & 0x0_0ff0) | (data << 12);
                break;
            case 2:
                chip.channel[channel].end = ((chip.channel[channel].end) & 0xf_f000) | (data << 4);
                break;
            case 3:
                chip.channel[channel].end = ((chip.channel[channel].end) & 0x0_0ff0) | (data << 12);
                break;
            case 4:
                chip.channel[channel].rate = 0x100_0000 / (256 - data);
                break;
            case 5:
    ...
    public static short irem_ga20_r(int offset) {
        int channel;
        short result;
        Sound.iremga20stream.stream_update();
        channel = offset >> 3;
        switch (offset & 0x7) {
            case 7:
                result = (short) (chip.channel[channel].play != 0 ? 1 : 0);
                break;
            default:
                result = 0;
                break;
