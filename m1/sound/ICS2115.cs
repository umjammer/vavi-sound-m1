...
namespace mame
{
    public class ICS2115
    {
        ...
        public static ushort[] volume = new ushort[4096];
        public static ics2115_voice[] voice;
        public struct osc_struct
        {
            public int left;
            public uint acc, start, end;
            public ushort fc;
        ...
        public class ics2115_voice
        {
            ...
            public bool update_volume_envelope()
            {
                ...
                if (vol_ctrl.loop)
                {
                    ...
                    if (vol_ctrl.invert)
                        vol.acc = (uint)(vol.end + vol.left);
                    else
                        vol.acc = (uint)(vol.start - vol.left);
                }
                else
                {
                    state.on = false;
            ...
            public bool update_oscillator()
            {
                ...
                if (osc_conf.loop)
                {
                    ...
                    if (osc_conf.invert)
                    {
                        osc.acc = (uint)(osc.end + osc.left);
                        osc.left = (int)(osc.acc - osc.start);
                    }
                    else
                    {
                        osc.acc = (uint)(osc.start - osc.left);
                        osc.left = (int)(osc.end - osc.acc);
                    }
                }
                else
        ...
        {
            uint curaddr = (uint)(((voice[osc].osc.saddr << 20) & 0xffffff) | (voice[osc].osc.acc >> 12));
            uint nextaddr;
            if (voice[osc].state.on && voice[osc].osc_conf.loop && !voice[osc].osc_conf.loop_bidir && (voice[osc].osc.left < (voice[osc].osc.fc << 2)))
            {
                nextaddr = (uint)(((voice[osc].osc.saddr << 20) & 0xffffff) | (voice[osc].osc.start >> 12));
            }
            else
                nextaddr = curaddr + 2;
        ...
        public static bool fill_output(int osc, int offset, int samples)
        {
            int i;
            bool irq_invalid = false;
            ushort fine = (ushort)(1 << (3 * (voice[osc].vol.incr >> 6)));
            voice[osc].vol.add = (uint)((voice[osc].vol.incr & 0x3f) << (10 - fine));
            for (i = 0; i < samples; i++)
            {
                uint volacc = (voice[osc].vol.acc >> 10) & 0xffff;
                uint volume2 = (uint)((volume[(int)(volacc >> 4)] * voice[osc].state.ramp) >> 6);
                ushort vleft = (ushort)volume2;
                ushort vright = (ushort)volume2;
                int sample;
        ...
        public static void reg_write(byte data, bool msb)
        {
            switch (reg_select)
            {
                ...
                case 0x02: // [osc] Wavesample loop start high
                    if (msb)
                        voice[osc_select].osc.start = (uint)((voice[osc_select].osc.start & 0x00ffffff) | (data << 24));
                    else
                        voice[osc_select].osc.start = (uint)((voice[osc_select].osc.start & 0xff00ffff) | (data << 16));
                    break;
                case 0x03: // [osc] Wavesample loop start low
                    if (msb)
                        voice[osc_select].osc.start = (uint)((voice[osc_select].osc.start & 0xffff00ff) | (data << 8));
                    break;
                case 0x04: // [osc] Wavesample loop end high
                    if (msb)
                        voice[osc_select].osc.end = (uint)((voice[osc_select].osc.end & 0x00ffffff) | (data << 24));
                    else
                        voice[osc_select].osc.end = (uint)((voice[osc_select].osc.end & 0xff00ffff) | (data << 16));
                    break;
                case 0x05: // [osc] Wavesample loop end low
                    if (msb)
                        voice[osc_select].osc.end = (uint)((voice[osc_select].osc.end & 0xffff00ff) | (data << 8));
                    break;
                ...
                case 0x0A: // [osc] Wavesample address high
                    ...
                    if (msb)
                        voice[osc_select].osc.acc = (uint)((voice[osc_select].osc.acc & 0x00ffffff) | (data << 24));
                    else
                        voice[osc_select].osc.acc = (uint)((voice[osc_select].osc.acc & 0xff00ffff) | (data << 16));
                    break;
                case 0x0B: // [osc] Wavesample address low
                    if (msb)
                        voice[osc_select].osc.acc = (uint)((voice[osc_select].osc.acc & 0xffff00ff) | (data << 8));
                    else
                        voice[osc_select].osc.acc = (uint)((voice[osc_select].osc.acc & 0xffffff00) | (data & 0xf8));
                    break;
                case 0x0C: // [osc] Pan
                    if (msb)
        ...
        public static ushort reg_read()
        {
            ...
            switch (reg_select)
            {
                ...
                case 0x02: // [osc] Wavesample loop start high
                    ...
                    ret = (ushort)((voice[osc_select].osc.start >> 16) & 0xffff);
                    break;
                case 0x03: // [osc] Wavesample loop start low
                    ret = (ushort)((voice[osc_select].osc.start >> 0) & 0xff00);
                    break;
                case 0x04: // [osc] Wavesample loop end high
                    ret = (ushort)((voice[osc_select].osc.end >> 16) & 0xffff);
                    break;
                case 0x05: // [osc] Wavesample loop end low
                    ret = (ushort)((voice[osc_select].osc.end >> 0) & 0xff00);
                    break;
                case 0x06: // [osc] Volume Increment
                    ret = voice[osc_select].vol.incr;
                    break;
                case 0x07: // [osc] Volume Start
                    ret = (ushort)(voice[osc_select].vol.start >> (10 + 8));
                    break;
                case 0x08: // [osc] Volume End
                    ret = (ushort)(voice[osc_select].vol.end >> (10 + 8));
                    break;
                case 0x09: // [osc] Volume accumulator
                    //ret = v->Vol.Acc;
                    ret = (ushort)(voice[osc_select].vol.acc >> (10));
                    break;
                case 0x0A: // [osc] Wavesample address
                    ret = (ushort)((voice[osc_select].osc.acc >> 16) & 0xffff);
                    break;
                case 0x0B: // [osc] Wavesample address
                    ret = (ushort)((voice[osc_select].osc.acc >> 0) & 0xfff8);
                    break;
                case 0x0C: // [osc] Pan
                    ret = (ushort)(voice[osc_select].vol.pan << 8);
                    break;
                case 0x0D: // [osc] Volume Envelope Control
                    if (vmode==0)
                        ret = (ushort)(voice[osc_select].vol_ctrl.irq ? 0x81 : 0x01);
                    else
                        ret = 0x01;
                    ret <<= 8;
                    break;
                case 0x0E: // Active Voices
                    ...
                    break;
                case 0x0F:
                    {// [osc] Interrupt source/oscillator
                        ret = 0xff;
                        for (int i = 0; i <= active_osc; i++)
                        {
                            ...
                            {
                                ret = (ushort)(i | 0xe0);
                                ret &= (ushort)(voice[i].vol_ctrl.irq_pending ? (~0x40) : 0xff);
                                ret &= (ushort)(voice[i].osc_conf.irq_pending ? (~0x80) : 0xff);
                                recalc_irq();
                                if (voice[i].osc_conf.irq_pending)
                                {
                                    voice[i].osc_conf.irq_pending = false;
                                    ret &= unchecked((ushort)(~0x80));
                                }
                                if (voice[i].vol_ctrl.irq_pending)
                                {
                                    voice[i].vol_ctrl.irq_pending = false;
                                    ret &= unchecked((ushort)(~0x40));
                                }
                                break;
                            }
                        }
                ...
                case 0x10: // [osc] Oscillator Control
                    ret = (ushort)(voice[osc_select].osc.ctl << 8);
                    break;
                case 0x11: // [osc] Wavesample static address 27-20
                    ret = (ushort)(voice[osc_select].osc.saddr << 8);
                    break;
        ...
        public static void ics2115_start()
        {
            ...
            for (i = 0; i < 4096; i++)
            {
                volume[i] = (ushort)(((0x100 | (i & 0xff)) << 6) >> (15 - (i >> 8)));
            }
            ushort[] lut = new ushort[8];
            ushort lut_initial = 33 << 2;
            for (i = 0; i < 8; i++)
